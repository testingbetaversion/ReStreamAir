#!/usr/bin/env python3
"""Emits the shared parity fixtures in both languages.

The C self-test (core/tests/) and the Swift parity self-test
(CoreParityFixtures.swift) must feed byte-identical input to the two
implementations, or a difference in the fixtures would look like a difference in
behaviour. Both files are generated from the tables below and committed; rerun
this script after editing them:

    python3 scripts/gen-parity-fixtures.py
"""
...
def write_swift(path):
    lines = [BANNER.replace("// ", "// ", 1), "",
             "enum CoreParityFixtures {"]

    for name, base, text in PLAYLISTS:
        camel = name.split("_")[0] + "".join(part.capitalize() for part in name.split("_")[1:])
        lines.append("    static let %sBase = %s" % (camel, swift_string(base)))
        lines.append("    static let %s = %s" % (camel, swift_string(text)))
        lines.append("")
    lines.append("    static let playlists: [(name: String, base: String, text: String)] = [")
    for name, base, text in PLAYLISTS:
        camel = name.split("_")[0] + "".join(part.capitalize() for part in name.split("_")[1:])
        lines.append('        ("%s", %sBase, %s),' % (name, camel, camel))
    lines.append("    ]")
    lines.append("")

    lines.append("    static let ffmpegCases: [(name: String, inputs: FFmpegResident.Inputs)] = [")
    for case in FFARGS:
        ids = ", ".join(swift_string(rep[0]) for rep in case["representations"])
        types = ", ".join("%s: %s" % (swift_string(rep[0]), swift_string(rep[1]))
                          for rep in case["representations"])
        lines.append('        ("%s", FFmpegResident.Inputs(' % case["name"])
        lines.append("            ffmpegPath: %s," % swift_string(FFMPEG_PATH))
        lines.append("            sourceURL: %s," % swift_string(case["source_url"]))
        lines.append("            kind: %s," % swift_string(case["kind"]))
        lines.append("            inputMode: %s," % swift_string(case["input_mode"]))
        lines.append("            outputMode: %s," % swift_string(case["output_mode"]))
        lines.append("            outputTarget: %s," % swift_string(case["output_target"]))
        lines.append("            decryptionKeys: %s," % swift_string(case["decryption_keys"]))
        lines.append("            headers: %s," % swift_string(case["headers"]))
        lines.append("            proxy: %s," % swift_string(case["proxy"]))
        lines.append("            segmentUrlParams: %s," % swift_string(case["segment_url_params"]))
        lines.append("            representationIds: [%s]," % ids)
        lines.append("            representationTypes: [%s]," % (types if types else ":"))
        lines.append("            tempDir: URL(fileURLWithPath: %s)," % swift_string(TEMP_DIR))
        lines.append("            outputPlaylist: URL(fileURLWithPath: %s)," % swift_string(OUTPUT_PLAYLIST))
        lines.append("            playlistSegments: %d," % PLAYLIST_SEGMENTS)
        lines.append("            segmentSeconds: %d" % SEGMENT_SECONDS)
        lines.append("        )),")
    lines.append("    ]")
    lines.append("}")
    lines.append("")
    ...
    with open(path, "w") as handle:
        handle.write("import Foundation\n\n" + "\n".join(lines))


root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
write_c(os.path.join(root, "core", "tests", "fixtures.h"))
write_swift(os.path.join(root, "CoreParityFixtures.swift"))
print("wrote core/tests/fixtures.h and CoreParityFixtures.swift")
