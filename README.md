...
**Direct source** (checkbox under the CDN mirrors field) makes the stream's play link (`/play/<id>/index.m3u8`, and `index.mpd`) answer with a `302` redirect straight to the source URL — or whichever CDN mirror a resident ffmpeg last rotated to — instead of proxying/remuxing anything through the panel. Same behavior as the always-available `/source/<id>` route (shown as `sourceUrl` in the editor's status box), but applied to the URL players already have, so you can flip an existing stream to direct delivery without re-pointing any client. The API-key gate still applies to the redirect itself; past that, the player talks to the origin on its own.
...
**Also implemented — `manifest` as failure recovery**: when a stream's manifest fetch fails on its primary URL **and** every configured CDN mirror, and its provider has a script configured, ReStreamAir runs `action=manifest` (with that stream's `ScriptParams`) to get a fresh `ManifestUrl`/`Cdn`/`Headers`, persists them onto the stream, and retries. This fires from two places: the m3u8 passthrough path retries inline on the request that hit the failure (which now also walks the CDN mirrors itself before giving up), and the worker supervisor triggers a background refresh once a `kind=mpd`/ffmpeg stream has needed two consecutive restarts (its own retries and CDN rotation having already been exhausted by then). Refreshes are throttled to one per stream per minute and never run concurrently for the same stream.

**Planned, not yet wired up**: the `cdm`/`heartbeat` actions and proactive session-manifest playback (calling `manifest` *before* the source expires rather than after it fails, plus the periodic heartbeat). Streams imported via channels/events already store every field the script reports (`SessionManifest`, `ScriptParams`, `CdmType`, `Autostart`, `Start`/`End`, ...) so nothing is lost. The rest of this section documents the full target protocol so scripts can be written against it now even though ReStreamAir doesn't call every action yet.
