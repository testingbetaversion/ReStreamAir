...
The panel's static files are found automatically when `public/` sits next to the working directory or one level up; `--root` is for everything else. **A binary on its own is also enough**: with no `public/` in sight, every launch refreshes the front-end from this repository into a cache directory outside the install — `~/Library/Caches/ReStreamAir/public` on macOS, `${XDG_CACHE_HOME:-~/.cache}/restreamair/public` on Linux, and a per-user directory under `$TMPDIR` when neither is writable (a hardened systemd unit, a container with no home). The replacement is downloaded completely before it becomes active; if GitHub is unavailable, the last complete cache is served so an offline restart still works. `--refresh-web` forces the cached copy even when a local `public/` exists, `--web-ref` pins a branch, tag or commit instead of `main`, `--no-download` forbids the fetch entirely, and `RESTREAMAIR_WEB_CACHE` puts the cache wherever you like. A `public/` next to the binary normally wins and a checkout's copy is never written over. Without a web root and without a download it serves `/ping` and says why on startup rather than pretending.
...
| **HLS segment (s)** | Real output segment duration. The internal transmuxer groups short CMAF fragments into approximately this duration without re-encoding; FFmpeg pipelines use the same value for `-hls_time`. Default and recommended value: 10s. |
...
| **Parallel downloads** | Persistent media connections the channel opens to the origin, **shared by every rendition of the stream** rather than allowed to each — one pool thread is one kept-alive TCP connection carrying many sequential segment requests, and it is also the in-flight request cap. Default 6, capped at 8. Changing it restarts the stream's download pool; every other field here is picked up on the next poll. |
...
| **Reduce manifest polling** | Off by default. Floors the poll period at one manifest read per segment the source produces. Enable it for an origin that answers `429`/`403`; it trades live-edge responsiveness for a lower request rate. |
...
A provider sets a default **Proxy**, generic **Headers**, and a **Downloader** for every request its streams make. It may also carry an **Error webhook**: paste a Discord webhook URL and errors from that provider's streams and script actions are delivered in the background. Alert bursts are collapsed to protect Discord from floods, and source URLs—which often contain session tokens—are redacted. Each stream can override the proxy and adds three header buckets on top: **Manifest headers**, **Media headers**, and **HLS key headers** (`kind=m3u8` only). **Use proxy for** scopes the proxy to any of Script / Manifest / Media independently. Proxies may be HTTP or SOCKS5 (`socks5h://user:pass@host:port`), and credentials embedded in the proxy URL are answered on a Basic-auth challenge. **Force IPv6** makes that provider's manifest and media requests use IPv6 only, failing rather than falling back to IPv4. **Rotate healthy proxies** checks each entry through normal traffic, round-robins healthy proxies, quarantines failures with exponential cooldown, and automatically retries them. With rotation off, configured order remains the priority while the same health checks and recovery retries still apply.
...
| `--refresh-web` | Fetch into the cache even when a local `public/` exists; the local directory is never overwritten. |
...
- **Connections are one pool per stream, with a floor for each rendition.** `parallelDownloads` (default 6, capped at 8) sizes a single download pool shared by video and audio rather than one pool each. One thread is one persistent connection, so this is literally how many media connections the channel holds open at the origin: at the default, six for the whole channel, not six per rendition. Manifest polling costs exactly one more: the director reads the MPD once per cycle for every rendition at once and hands each one its own segment plan, so renditions have no poller — and no connection — of their own. Per-connection rate on a proxied path is roughly fixed, so aggregate throughput is very nearly a function of how many connections are open — a 3 Mbps rendition wanting ~370 KB/s is six connections at that rate and simply unobtainable from two. Each rendition reserves a slot only for the *other* renditions that actually have work waiting, and may use everything else: audio can't be shut out by video's much deeper queue, and isn't held down to one connection when video is idle. Note that this is the engine's budget — the stream's own `parallelDownloads` still caps it.
...
- **Reduce manifest polling** (`reducedManifestPolling`, off by default) floors the poll period at the observed segment duration, so the manifest is never read more often than the source produces media. It used to mean "back the rendition-list poll off so it stops being a third concurrent manifest fetch"; there is only one manifest reader now, so it means the thing its name always said.
...
**A concurrent-stream limit trips with only one stream running.** Count the connections: a stream holds `parallelDownloads` media connections plus exactly one manifest reader, whatever the number of renditions. Lower **Parallel downloads** to fit under the cap.
