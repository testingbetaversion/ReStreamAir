...
Ported so far: SHA-256/HMAC/PBKDF2, AES (block, CTR, CBC), RFC 3986 URL resolution, the HLS playlist rewriter, and the ffmpeg argument builder. Three call sites already run on it — admin password hashing, HLS AES-128 decryption, and CENC subsample decryption. Each of those used to fork on `#if canImport(CommonCrypto)`; each is now one implementation everywhere, and the app no longer links CommonCrypto at all.
...
Every `/api/*` endpoint except `/api/auth/*` needs a signed-in session cookie or an `Authorization: Basic` header. Add more admins from the **Users** panel. **Remember me** gives a 30-day persistent cookie; unchecked gives a browser-session cookie. Server-side sessions are capped at 24h either way.
...
Streams also export as M3U for external players: the list icon on a provider card (`GET /api/providers/<id>/playlist.m3u8`), or **Export playlist (all)** (`GET /api/playlist.m3u8`). Entries carry `group-title` and `tvg-logo`. The playlist lists every stream, but opening one doesn't start it — `kind=mpd` streams must be **Start**ed from the panel first, or the URL 404s.
